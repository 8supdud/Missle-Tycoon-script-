--[[
    ...
    VOZEX HUB - RIVALS SCRIPT (FULLY WORKING)
    Release Ready | Mobile/PC Friendly
--]]
...
-- Warning function for risky features
local function showBanWarning(featureName)
    ...
    msg.Size = UDim2.new(1, -70, 0, 50)
    msg.Position = UDim2.new(0, 60, 0, 45)
...
function VozexUI:CreateWindow(opts)
    ...
    function WindowObj:Notify(opts)
        ...
        notif.Position = UDim2.new(1, -340, 0, 60)
        ...
        task.delay(opts.Duration or 3, function()
            TweenService:Create(notif, TweenInfo.new(0.3), {Position = UDim2.new(1, -340, 0, -100)}):Play()
            task.wait(0.3)
            notif:Destroy()
        end)
        
        TweenService:Create(notif, TweenInfo.new(0.3, Enum.EasingStyle.Back), {Position = UDim2.new(1, -340, 0, 60)}):Play()
    end
...
-- ============================================
-- MOVEMENT TAB (WITH WORKING WARNINGS)
-- ============================================
...
MovementTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then 
            showBanWarning("Walkspeed modification")
            Window:Notify({Title = "⚠️ Warning", Content = "Walkspeed can get you banned! Use at your own risk.", Duration = 3})
        end
...
MovementTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then 
            showBanWarning("Jump Power modification")
            Window:Notify({Title = "⚠️ Warning", Content = "Jump Power can get you banned! Use at your own risk.", Duration = 3})
        end
...
MovementTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then 
            showBanWarning("Noclip")
            Window:Notify({Title = "⚠️ Warning", Content = "Noclip can get you banned! Use at your own risk.", Duration = 3})
        end
...
MovementTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then 
            showBanWarning("Infinite Jump")
            Window:Notify({Title = "⚠️ Warning", Content = "Infinite Jump can get you banned! Use at your own risk.", Duration = 3})
        end
...
MovementTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then 
            showBanWarning("Fly")
            Window:Notify({Title = "⚠️ Warning", Content = "Fly can get you banned! Use at your own risk.", Duration = 3})
        end
...
║         Mobile/PC Friendly | Fully Featured                  ║
║                   Release Ready!                             ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
]])
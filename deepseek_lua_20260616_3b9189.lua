...
-- Find specific remotes by scanning all
local function findRemoteByName(partialName)
    for _, remote in ipairs(getAllRemotes()) do
        if remote.Name:lower():find(partialName:lower()) then
            return remote
        end
    end
    return nil
...
-- Find all relevant remotes
local function findGardenRemotes()
    local results = {}
    for _, remote in ipairs(getAllRemotes()) do
        local name = remote.Name:lower()
        if name:find("harvest") or name:find("collect") or name:find("pick") or 
           name:find("plant") or name:find("seed") or name:find("garden") or
           name:find("grow") or name:find("water") or name:find("farm") or
           name:find("plot") or name:find("shop") or name:find("buy") or
           name:find("purchase") then
...
-- Try to find harvest remote by scanning workspace interactions
local HarvestRemote = findRemoteByName("Harvest") or findRemoteByName("Collect") or findRemoteByName("Pick")
local PlantRemote = findRemoteByName("Plant") or findRemoteByName("Seed") 
local PurchaseRemote = findRemoteByName("Buy") or findRemoteByName("Purchase") or findRemoteByName("Shop")

-- =========================================================================
-- GARDEN DETECTION (Based on the actual game structure from console output)
-- =========================================================================
local function getPlants()
    local plants = {}
    -- The plants have IDs like "2952811691_35761e73-9a45-483a-91c1-46dd756595e"
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") then
            -- Check for plant characteristics from console output
            if obj:GetAttribute("Age") ~= nil and obj:GetAttribute("MaxAge") ~= nil then
                table.insert(plants, obj)
            elseif obj.Name:match("%d+_%x+%-%x+%-%x+%-%x+%-%x+") then
                -- Plants in this game have unique IDs like the ones shown in console
                if obj:FindFirstChild("Base") or obj:FindFirstChildOfClass("BasePart") then
                    table.insert(plants, obj)
                end
            end
        end
...
local function getReadyPlants()
    local readyPlants = {}
    for _, plant in ipairs(getPlants()) do
        local age = plant:GetAttribute("Age") or 0
        local maxAge = plant:GetAttribute("MaxAge") or 999
        if age >= maxAge then
...
local function getEmptyPlots()
    ...
    for _, plant in ipairs(plants) do
        local parent = plant.Parent
        if parent and parent:IsA("Model") then
            plotNames[parent.Name] = true
        end
    end
    
    -- Find plots in workspace
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and (obj.Name:find("Plot") or obj.Name:find("Garden") or obj:FindFirstChild("Base")) then
            if not plotNames[obj.Name] then
                -- Double check no plant exists
                local hasPlant = false
                for _, child in ipairs(obj:GetChildren()) do
                    if child:IsA("Model") and child:GetAttribute("Age") ~= nil then
                        hasPlant = true
                        break
                    end
                end
...
-- =========================================================================
-- HARVEST FUNCTIONS - MULTIPLE METHODS
-- =========================================================================
local function harvestByRemote(plant)
    ...
    local methods = {
        ...
        function() 
            if HarvestRemote:IsA("RemoteFunction") then
                return HarvestRemote:InvokeServer("Harvest", plant)
            else
        ...
        function()
            if HarvestRemote:IsA("RemoteFunction") then
                return HarvestRemote:InvokeServer(plant.Name)
            else
                HarvestRemote:FireServer(plant.Name)
            end
        end
    }
...
local function harvestByClick(plant)
    -- Try to find a clickable part on the plant
    local clickPart = plant:FindFirstChild("ClickDetector") or plant:FindFirstChildOfClass("ClickDetector")
    if clickPart then
        pcall(function()
            clickPart:Click()
        end)
    ...
    -- Try to find any part with a click detector
...
local function harvestByProximity(plant)
...
local function harvestPlant(plant)
    ...
    print("[VOZEX] Attempting to harvest:", plant.Name, "Age:", plant:GetAttribute("Age"), "/", plant:GetAttribute("MaxAge"))
    
    -- Try all harvest methods
    local success = harvestByRemote(plant) or harvestByClick(plant) or harvestByProximity(plant)
    
    if success then
        print("[VOZEX] Successfully harvested:", plant.Name)
    else
        print("[VOZEX] Failed to harvest:", plant.Name)
        -- Try scanning for fruit spawn locations and clicking them
        local fruitSpawns = plant:FindFirstChild("FruitSpawnLocations")
        if fruitSpawns then
            for _, spawn in ipairs(fruitSpawns:GetChildren()) do
                if spawn:IsA("BasePart") then
                    pcall(function()
                        -- Try to click on the fruit spawn
...
local function plantSeed(plot, seedId)
    if not PlantRemote then return false end
    ...
    local methods = {
        ...
        function()
            if PlantRemote:IsA("RemoteFunction") then
                return PlantRemote:InvokeServer("Plant", plot, seedId)
            else
        ...
        function()
            if PlantRemote:IsA("RemoteFunction") then
                return PlantRemote:InvokeServer(seedId, plot)
            else
                PlantRemote:FireServer(seedId, plot)
            end
        end
    }
...
local function buySeed(seedId)
    ...
    local methods = {
        ...
        function()
            if PurchaseRemote:IsA("RemoteFunction") then
                return PurchaseRemote:InvokeServer("Purchase", seedId, 1)
            else
        ...
        function()
            if PurchaseRemote:IsA("RemoteFunction") then
                return PurchaseRemote:InvokeServer(seedId, 1)
            else
...
local function autoHarvestLoop()
    while _G.AutoHarvest do
        ...
        print("[VOZEX] Found", #readyPlants, "ready plants to harvest")
        
        if #readyPlants > 0 then
...
-- =========================================================================
-- VOZEX UI - Same as before but keeping it clean
-- =========================================================================
local VozexUI = {}
VozexUI.Colors = {
...
function VozexUI:CreateWindow(opts)
    ...
    if isMobile then
        main.Size = UDim2.new(0, 600 * 0.85, 0, 520 * 0.85)
    else
...
GardenTab:CreateToggle({
    ...
    Callback = function(v)
        _G.AutoHarvest = v
        if v then
            print("[VOZEX] Auto Harvest STARTED")
            task.spawn(autoHarvestLoop)
        else
            print("[VOZEX] Auto Harvest STOPPED")
        end
    end
})
...
GardenTab:CreateToggle({
    ...
    Callback = function(v)
        _G.AutoPlant = v
        if v then
            print("[VOZEX] Auto Plant STARTED")
            task.spawn(autoPlantLoop)
        else
            print("[VOZEX] Auto Plant STOPPED")
        end
    end
})

GardenTab:CreateSection("Delay Configuration")
...
local statusLabel = GardenTab:CreateLabel("🔍 Scanning garden...", VozexUI.Colors.TextDim)

task.spawn(function()
    while true do
        local readyCount = #getReadyPlants()
        local emptyCount = #getEmptyPlots()
        local allPlants = #getPlants()
            ...
            if child:IsA("ScrollingFrame") and child.Visible then
                for _, grandchild in ipairs(child:GetChildren()) do
                    if grandchild:IsA("TextLabel") and grandchild.Text:find("🔍") then
                        grandchild.Text = string.format("🔍 Ready: %d | Empty: %d | Total Plants: %d", readyCount, emptyCount, allPlants)
                    end
                end
            end
        end
...
DebugTab:CreateSection("Remote Detection")
...
DebugTab:CreateToggle({
    ...
    Callback = function(v)
        if v then
            ...
            print("\nAll Garden-Related Remotes:")
            local foundRemotes = findGardenRemotes()
            for _, remote in ipairs(foundRemotes) do
                print("  -", remote.FullName)
            end
            ...
            for _, plant in ipairs(plants) do
                ...
                print("  -", plant.Name, "Age:", age, "/", maxAge)
            end
            print("==========================")
        end
    end
...
DebugTab:CreateToggle({
    Name = "🔎 Scan Garden Now",
    CurrentValue = false,
    Callback = function(v)
        if v then
            print("[VOZEX] Manual Garden Scan:")
            local ready = getReadyPlants()
            local empty = getEmptyPlots()
            local allPlants = getPlants()
            
            print(string.format("Found %d ready plants, %d empty plots, %d total plants", #ready, #empty, #allPlants))
            ...
            if #ready > 0 then
                ...
                for i, plant in ipairs(ready) do
                    local age = plant:GetAttribute("Age") or 0
                    local maxAge = plant:GetAttribute("MaxAge") or 0
                    print(string.format("  %d. %s (Age: %d/%d)", i, plant.Name, age, maxAge))
                end
            end
            _G.ScanNow = false
        end
...
DebugTab:CreateToggle({
    Name = "🧪 Test Harvest (First Ready Plant)",
    CurrentValue = false,
    Callback = function(v)
        if v then
            local ready = getReadyPlants()
            if #ready > 0 then
                print("[VOZEX] Testing harvest on:", ready[1].Name)
                harvestPlant(ready[1])
            else
                print("[VOZEX] No ready plants found to test harvest")
            end
            _G.TestHarvest = false
        end
    end
})

DebugTab:CreateSection("Info")

DebugTab:CreateLabel("Check output console for details")

-- =========================================================================
-- INIT COMPLETE
-- =========================================================================
Window:Notify({Title = "Vozex Hub Loaded ✅", Content = "Check console for remote info. Use Debug tab if features don't work."})

print("[VOZEX HUB] ========================================")
print("[VOZEX HUB] Grow A Garden 2 Automation Loaded")
print("[VOZEX HUB] Harvest Remote:", HarvestRemote and HarvestRemote.Name or "NOT FOUND")
print("[VOZEX HUB] Plant Remote:", PlantRemote and PlantRemote.Name or "NOT FOUND")
print("[VOZEX HUB] Purchase Remote:", PurchaseRemote and PurchaseRemote.Name or "NOT FOUND")
print("[VOZEX HUB] ========================================")
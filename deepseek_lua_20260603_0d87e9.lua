...
task.spawn(function()
    ...
    local MobileScale = 0.8 

    local gui = nil
    while not gui do
        gui = CoreGui:FindFirstChild("VOZEX_HUB") 
        if not gui and Players.LocalPlayer then
            local pg = Players.LocalPlayer:FindFirstChild("PlayerGui")
            if pg then gui = pg:FindFirstChild("VOZEX_HUB") end
        end
        task.wait(0.3)
    end

    local rootFrame = gui:WaitForChild("Frame", 10)
    if not rootFrame then return end

    if isMobile then
        rootFrame.Size = UDim2.new(rootFrame.Size.X.Scale * (1/MobileScale), rootFrame.Size.X.Offset * MobileScale, 
                                   rootFrame.Size.Y.Scale * (1/MobileScale), rootFrame.Size.Y.Offset * MobileScale)
    ...
    local function refineUIElement(element)
        ...
        if isMobile and element:IsA("TextButton") and string.find(element.Name, "_Bind") then
            local blocker = Instance.new("TextButton")
            blocker.Size = UDim2.new(1, 0, 1, 0)
            blocker.BackgroundTransparency = 1
            blocker.Text = ""
            blocker.ZIndex = element.ZIndex + 10
            blocker.Parent = element
            
            task.spawn(function()
                while task.wait(1) do
                    if element.Text ~= "[ PC Keybind ]" then
                        element.Text = "[ PC Keybind ]"
                    end
                end
            end)
        end
    ...
    for _, child in ipairs(rootFrame:GetDescendants()) do
        refineUIElement(child)
    end
    rootFrame.DescendantAdded:Connect(refineUIElement)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            ...
            local padding = 15
            local minX = padding - (rootFrame.AbsoluteSize.X * rootFrame.AnchorPoint.X)
            local maxX = screen.X - padding - (rootFrame.AbsoluteSize.X * (1 - rootFrame.AnchorPoint.X))
            local minY = padding - (rootFrame.AbsoluteSize.Y * rootFrame.AnchorPoint.Y)
            local maxY = screen.Y - padding - (rootFrame.AbsoluteSize.Y * (1 - rootFrame.AnchorPoint.Y))
            
            targetX = math.clamp(targetX, minX, maxX)
            targetY = math.clamp(targetY, minY, maxY)
            
            rootFrame.Position = UDim2.new(framePos.X.Scale, targetX, framePos.Y.Scale, targetY)
        end
    end)

    if isMobile then
        local toggleBtn = Instance.new("TextButton")
        toggleBtn.Name = "MobileToggleMenu"
        toggleBtn.Size = UDim2.new(0, 45, 0, 45)
        toggleBtn.Position = UDim2.new(0.5, -22, 0, 15)
        toggleBtn.BackgroundColor3 = Color3.fromRGB(0, 212, 255)
        toggleBtn.Text = "👑"
        ...
        aimBtn.Name = "MobileAimLock"
        aimBtn.Size = UDim2.new(0, 55, 0, 55)
        aimBtn.Position = UDim2.new(1, -70, 0.5, 0)
        aimBtn.BackgroundColor3 = Color3.fromRGB(20, 18, 35)
        aimBtn.Text = "🎯\nAim"
        aimBtn.TextSize = 14
        aimBtn.Font = Enum.Font.GothamBold
        aimBtn.TextColor3 = Color3.fromRGB(0, 212, 255)
        aimBtn.Parent = gui
        ...
        aimStroke.Color = Color3.fromRGB(0, 212, 255)
        ...
        aimBtn.MouseButton1Click:Connect(function()
            ...
            if aimLocked then
                aimBtn.BackgroundColor3 = Color3.fromRGB(0, 212, 255)

...
-- Find remotes based on the game's structure
local function findGardenRemotes()
    local remotes = {}
    
    -- Search for plant/harvest related remotes
    for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
        if remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") then
            local name = remote.Name:lower()
            if name:find("plant") or name:find("harvest") or name:find("garden") or 
               name:find("grow") or name:find("seed") or name:find("water") then
...
local HarvestRemote = findRemote("Harvest") or findRemote("Collect")
local PlantRemote = findRemote("Plant") or findRemote("Seed")
local PurchaseRemote = findRemote("Buy") or findRemote("Purchase") or findRemote("Shop")

-- =========================================================================
-- GARDEN OBJECT DETECTION (Based on the Apple module structure)
-- =========================================================================
local function getGardenPlots()
    local plots = {}
    -- Look for plot models in workspace
...
local function getPlants()
    local plants = {}
    -- Look for plant models with the Apple module structure
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and obj:FindFirstChild("Base") then
            -- Check if it has plant attributes (Age, MaxAge from the module)
...
local function getReadyPlants()
    ...
    for _, plant in ipairs(getPlants()) do
        ...
        if age >= maxAge then
            -- Also check if it has fruit spawn locations
            if plant:FindFirstChild("FruitSpawnLocations") then
...
local function getEmptyPlots()
    ...
    -- Get names of plots that have plants
    for _, plant in ipairs(allPlants) do
        -- Check if plant has a parent that's a plot
        local parent = plant.Parent
        if parent and parent:IsA("Model") and (parent.Name:find("Plot") or parent.Name:find("Garden")) then
            plotNames[parent.Name] = true
        end
    end
    
    -- Find empty plots
    for _, plot in ipairs(getGardenPlots()) do
        if not plotNames[plot.Name] then
            -- Double check if plot has any plant children
            local hasPlant = false
            for _, child in ipairs(plot:GetChildren()) do
                if child:IsA("Model") and child:FindFirstChild("Base") then
                    if child:GetAttribute("Age") ~= nil then
                        hasPlant = true
                        break
                    end
                end
...
local function plantSeed(plot, seedId)
    if PlantRemote and plot and seedId then
        pcall(function()
            if PlantRemote:IsA("RemoteFunction") then
                ...
                PlantRemote:FireServer(plot, seedId, 1)
            end
        end)
        return true
    end
...
task.spawn(function()
    while true do
        ...
        local totalPlots = #getGardenPlots()
            ...
            if child:IsA("ScrollingFrame") and child.Visible then
                for _, grandchild in ipairs(child:GetChildren()) do
                    if grandchild:IsA("TextLabel") and grandchild.Text:find("🔍") then
                        grandchild.Text = string.format("🔍 Ready: %d | Empty: %d | Total: %d", readyCount, emptyCount, totalPlots)
                    end
                end
            end
        end
...
DebugTab:CreateToggle({
    ...
    Callback = function(v)
        if v then
            ...
            local total = getGardenPlots()
            
            print(string.format("Found %d ready plants, %d empty plots, %d total plots", #ready, #empty, #total))
...
DebugTab:CreateSection("Plant Info")

DebugTab:CreateLabel("Check output console for details")

Window:Notify({Title = "Vozex Hub Loaded ✅", Content = "Garden automation ready. Use Debug tab if features don't work."})

print("[VOZEX HUB] Successfully loaded Grow A Garden 2 automation")
print("[VOZEX HUB] Found remotes - Harvest:", HarvestRemote and HarvestRemote.Name or "Not found")
print("[VOZEX HUB] Found remotes - Plant:", PlantRemote and PlantRemote.Name or "Not found")
print("[VOZEX HUB] Found remotes - Purchase:", PurchaseRemote and PurchaseRemote.Name or "Not found")